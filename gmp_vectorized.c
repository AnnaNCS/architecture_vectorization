...
// Number of elements processed in each iteration
#define ITER (VECT_SIZE / 4) 
...
void print_m256i(__m256i vec) {
    // Allocate an array to store the vector elements
    int32_t elements[8];
    // Store the vector elements in the array
...
uint32_t mult_vect(uint32_t *arrayA, uint32_t *arrayB, size_t size) {

    // Initialize result to zero
    __m256i result = _mm256_setzero_si256();
    // Testing and ensuring the array is zeroed out
    // print_m256i(result);

    size_t i;
    for (i = 0; i + 8 <= size; i += 8) {
        // Load 8 elements from arrayA and arrayB into AVX2 registers
        __m256i vecA = _mm256_loadu_si256((__m256i*) &arrayA[i]);
        __m256i vecB = _mm256_loadu_si256((__m256i*) &arrayB[i]);
        // printf("Printing the values to be multiplied: \n");
        // print_m256i(vecA);
        // print_m256i(vecB);

        // Multiply arrayA and arrayB elements
        __m256i multiplied = _mm256_mullo_epi32(vecA, vecB);

        // Accumulate the results
        result = _mm256_add_epi32(result, multiplied);
        // print_m256i(result);
    }
    __m256i remainingResult = _mm256_setzero_si256();

    if (i < size) {
        // Load the remaining elements from arrayA and arrayB into AVX2 registers
        __m256i vecA = _mm256_maskload_epi32((const int *)&arrayA[i], _mm256_set_epi32(0, 0, 0, 0, 0, 0, 0, -1));
        __m256i vecB = _mm256_maskload_epi32((const int *)&arrayB[i], _mm256_set_epi32(0, 0, 0, 0, 0, 0, 0, -1));

        // Perform multiplication
        __m256i multiplied = _mm256_mullo_epi32(vecA, vecB);

        // Add the multiplied values to the remaining result
        remainingResult = _mm256_add_epi32(remainingResult, multiplied);
    }

    // print_m256i(remainingResult);
    // Combine the overall result (including the remaining elements)
    result = _mm256_add_epi32(result, remainingResult);
    // print_m256i(result);

    // Sum the elements in the result vector
    __m128i sum_low  = _mm256_castsi256_si128(result);
    __m128i sum_high = _mm256_extracti128_si256(result, 1);
    ...
    _mm_store_ss((float*)&finalResult, _mm_castsi128_ps(sum));
...
int main(void) {
    ...
    _mm_mfence();
    start = rdtsc();
    result_vect = mult_vect(a, b, VECT_SIZE);
    // stop count
    end = rdtsc();
    _mm_mfence();
